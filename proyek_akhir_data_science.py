# -*- coding: utf-8 -*-
"""Proyek Akhir Data Science.ipynb

Automatically generated by Colab.

Original file is located at
    https://colab.research.google.com/drive/1GozZQ9oVngrgI_S3V6Vgyvb4uYeiIu29
...
encoded_df = one_hot_encoding_categorical_features(new_df, columns_to_encode)
...
def encoding(features, df):
    ...
    for feature in features:
        ...
        # There seems to be a syntax error in the original fit call.
        # It should be encoder.fit(df[feature]).
        # Assuming the intention is to fit on the column data.
...
new_df_scaled = scaling(numerical_columns, encoded_df)
new_df_processed = encoding(categorical_columns, new_df_scaled)

encoder_status = LabelEncoder()
new_df_processed['Status'] = encoder_status.fit_transform(new_df_processed['Status'])

joblib.dump(encoder_status, "model/encoder_target.joblib")
...
train_df, test_df = train_test_split(new_df_processed, test_size=0.2, random_state=42, shuffle=True)
...
train_pca_df = X_train.copy().reset_index(drop=True) # X_train is the preprocessed training data for PCA
test_pca_df = X_test.copy().reset_index(drop=True)   # X_test is the preprocessed test data for PCA
...
encoder = joblib.load('/content/model/encoder_target.joblib')
